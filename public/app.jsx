...
function PuzzleScreen({onComplete,hardMode,day,c,user}){
  ...
  useEffect(()=>{setSelected(null);setTypedAnswer("");setSubmitted(false);setResultAnim(null);setRevealedAnswer(null);setTimeLeft(q.time_limit||null);},[qIdx]);
  useEffect(()=>{
  ...
  function handleSubmit(timedOut=false){
    ...
    // If we have a question ID (from API), check answer server-side
    if(q.id&&apiQuestions){
      apiFetch("/puzzle/check-answer",{method:"POST",body:JSON.stringify({questionId:q.id,userAnswer:userAns||""})})
        .then(data=>{
          const correct=data.correct;
          const answer=data.answer;
          correctAnswersRef.current[q.id]=answer;
          setRevealedAnswer(answer);
          const pts=calcScore(correct,timeLeft??0,q.time_limit??0,streak);
          const newScores=[...scores,{correct,pts,question_id:q.id}];
        ...
        .catch(()=>{
          ...
          const correct=q.answer?(q.type==="type_in"?fuzzyMatch(userAns||"",q.answer):userAns===q.answer):false;
  ...
  // Use revealedAnswer for display (from API or local), fall back to q.answer for local-only mode
  const displayAnswer=revealedAnswer||q.answer||"";
  // hasResult = true only once the API (or local fallback) has returned
  const hasResult=resultAnim!==null;
  // For UI: treat submitted-but-waiting as a neutral "checking" state
  const showCorrect=hasResult&&resultAnim==="correct";
  const showWrong=hasResult&&resultAnim==="wrong";
  ...
  return(
    <div style={{minHeight:"100vh",background:c.bg}}>
      <div style={{padding:"16px 20px 12px",background:c.surface,borderBottom:`1.5px solid ${c.border}`,display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:4,marginTop:4}}>
            {questions.map((_,i)=><div key={i} style={{width:28,height:6,borderRadius:3,background:i<qIdx?c.accent:i===qIdx?(hasResult?(showCorrect?c.correct:c.wrong):c.accent):c.border,transition:"background 0.3s"}}/>)}
          </div>
        </div>
      ...
      <div style={{padding:"20px 20px 0"}}>
        <Card c={c} style={{marginBottom:16,background:hasResult?(showCorrect?c.correctBg:c.wrongBg):c.surface,border:`1.5px solid ${hasResult?(showCorrect?c.correct:c.wrong):c.border}`,transition:"all 0.3s",position:"relative"}}>
          {hasResult&&<div style={{position:"absolute",top:16,right:16}}>{showCorrect?<Icon.Check size={32} color={c.correct}/>:<Icon.Cross size={32} color={c.wrong}/>}</div>}
          {submitted&&!hasResult&&<div style={{position:"absolute",top:16,right:16}}><div style={{width:28,height:28,borderRadius:"50%",border:"3px solid "+c.border,borderTopColor:c.accent,animation:"spin 0.8s linear infinite"}}/></div>}
        ...
        {(q.type==="multiple_choice"||q.type==="timed")&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {q.options.map(opt=>{
              const isSel=selected===opt,isCorr=hasResult&&opt===displayAnswer,isWrong=hasResult&&isSel&&opt!==displayAnswer;
              return (
        ...
        {q.type==="type_in"&&(
          <div>
            ...
            {hasResult&&displayAnswer&&<div style={{marginTop:8,fontSize:14,color:c.textMuted}}>Accepted answer: <strong style={{color:c.correct}}>{displayAnswer}</strong></div>}
          </div>
        )}
            ...
            <div key={i} style={{padding:"14px 16px",borderTop:i>0?`1px solid ${c.border}`:"none",background:b.correct?c.correctBg:c.wrongBg}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {!b.correct&&q&&(q.answer||b.correctAnswer)&&<div style={{fontSize:12,color:c.correct,fontWeight:600}}>✓ {q.answer||b.correctAnswer}</div>}
                </div>
...
// ── PROFILE SCREEN ────────────────────────────────────────────────────────
function ProfileScreen({profile,groups,onBack,c,user,onUpdateUser,onLogout}){
    ...
    <div style={{minHeight:"100vh",background:c.bg}}>
      ...
      <div style={{padding:"20px 20px 48px"}}>
        ...
        <div style={{marginTop:12,marginBottom:28}}>
          <Btn label="Log Out" icon={<Icon.Leave size={18} color={c.wrong}/>} onClick={onLogout} variant="ghost" c={c} style={{borderColor:c.wrong,color:c.wrong}}/>
        </div>
      </div>
    </div>
  );
...
function OnboardingScreen({onComplete}){
  ...
  const inputStyle={width:"100%",padding:"15px 18px",borderRadius:12,border:"2px solid #2D4A5A",background:"#0d2535",color:"#EEEEEE",fontSize:16,outline:"none",boxSizing:"border-box",caretColor:"#00ADB5",marginBottom:10};
  ...
  async function handleSignup(){
    ...
    if(trimmed.length>24){setError("Name max 24 characters.");return;}
    if(!email.trim()){setError("Email is required.");return;}
    if(!password||password.length<6){setError("Password must be at least 6 characters.");return;}
    setError("");setLoading(true);
    try{
      const data=await api.signup(email.trim(),password,trimmed);
      const u={id:data.user.id,handle:data.user.handle||trimmed,publicName:data.user.public_name||publicName,email:data.user.email||email.trim()};
      try{localStorage.setItem("uncl_user",JSON.stringify(u));}catch(e){}
      onComplete(u);
    }catch(e){
  ...
  async function handleLogin(){
    ...
    try{
      ...
      const u={id:data.user.id,handle:data.user.handle,publicName:data.user.public_name,email:data.user.email};
      try{localStorage.setItem("uncl_user",JSON.stringify(u));}catch(e){}
      onComplete(u);
    }catch(e){
  ...
  if(mode==="welcome"){
    return(
      <div style={{...BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 28px"}}>
        ...
        <button onClick={()=>{setMode("signup");setError("");}} style={{width:"100%",maxWidth:320,padding:"16px 0",borderRadius:12,background:"#00ADB5",border:"none",color:"#fff",fontSize:17,fontWeight:700,cursor:"pointer",letterSpacing:0.2,boxShadow:"0 4px 20px rgba(0,173,181,0.4)",marginBottom:12}}>Create Account</button>
        <button onClick={()=>{setMode("login");setError("");}} style={{width:"100%",maxWidth:320,padding:"14px 0",borderRadius:12,background:"transparent",border:"2px solid #2D4A5A",color:"#9CA3AF",fontSize:15,fontWeight:600,cursor:"pointer"}}>I already have an account</button>
      </div>
    );
  }
  ...
  if(mode==="login"){
    return(
      <div style={{...BG,display:"flex",flexDirection:"column",alignItems:"center",padding:"60px 24px 40px"}}>
        ...
        <div style={{fontSize:24,fontWeight:900,color:"#EEEEEE",marginBottom:6}}>Welcome back</div>
        <div style={{fontSize:14,color:"#9CA3AF",marginBottom:32,textAlign:"center",lineHeight:1.5,maxWidth:300}}>Log in to continue your streak and see your groups.</div>
        <div style={{width:"100%",maxWidth:360}}>
          <input value={email} onChange={e=>{setEmail(e.target.value);setError("");}} placeholder="Email" type="email" style={error?inputErrorStyle:inputStyle} autoFocus/>
          <input value={password} onChange={e=>{setPassword(e.target.value);setError("");}} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder="Password" type="password" style={error?inputErrorStyle:inputStyle}/>
          {error&&<div style={{color:"#EF4444",fontSize:13,marginBottom:10,paddingLeft:4}}>{error}</div>}
          <button onClick={handleLogin} disabled={loading} style={{width:"100%",padding:"15px 0",borderRadius:12,background:"#00ADB5",border:"none",color:"#fff",fontSize:16,fontWeight:700,cursor:loading?"default":"pointer",opacity:loading?0.6:1,marginBottom:12}}>
            {loading?"Logging in...":"Log In"}
          </button>
          <button onClick={()=>{setMode("signup");setError("");setPassword("");}} style={{width:"100%",padding:"12px 0",borderRadius:12,background:"transparent",border:"none",color:"#6B7280",fontSize:14,cursor:"pointer"}}>
            Don't have an account? <span style={{color:"#00ADB5",fontWeight:600}}>Sign up</span>
          </button>
          <button onClick={()=>{setMode("welcome");setError("");}} style={{width:"100%",padding:"8px 0",background:"transparent",border:"none",color:"#4A6070",fontSize:13,cursor:"pointer",marginTop:4}}>← Back</button>
        </div>
      </div>
    );
  }
  ...
  // ── Signup screen ──
  return(
    <div style={{...BG,display:"flex",flexDirection:"column",alignItems:"center",padding:"48px 24px 40px"}}>
      <div style={{marginBottom:20}}><AppLogo size={70}/></div>
      <div style={{fontSize:22,fontWeight:900,color:"#EEEEEE",marginBottom:6}}>Create your account</div>
      <div style={{fontSize:14,color:"#9CA3AF",marginBottom:28,textAlign:"center",lineHeight:1.5,maxWidth:300}}>Pick a name your friends will recognise, and add your email so you can log in from any device.</div>
      <div style={{width:"100%",maxWidth:360}}>
        <div style={{fontSize:12,fontWeight:600,color:"#4A8090",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Display Name</div>
        <input value={handle} onChange={e=>{setHandle(e.target.value);setError("");}} placeholder="e.g. Jordan, Coach K, The Riddler…" maxLength={24} autoFocus style={inputStyle}/>
        <div style={{fontSize:12,fontWeight:600,color:"#4A8090",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Email</div>
        <input value={email} onChange={e=>{setEmail(e.target.value);setError("");}} placeholder="you@example.com" type="email" style={inputStyle}/>
        <div style={{fontSize:12,fontWeight:600,color:"#4A8090",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Password</div>
        <input value={password} onChange={e=>{setPassword(e.target.value);setError("");}} onKeyDown={e=>e.key==="Enter"&&handleSignup()} placeholder="At least 6 characters" type="password" style={inputStyle}/>
        {error&&<div style={{color:"#EF4444",fontSize:13,marginBottom:10,paddingLeft:4}}>{error}</div>}

        <div style={{background:"#0d2535",border:"1.5px solid #2D4A5A",borderRadius:12,padding:"14px 16px",marginBottom:16}}>
          <div style={{fontSize:10,fontWeight:700,color:"#00ADB5",textTransform:"uppercase",letterSpacing:1.5,marginBottom:6}}>Your anonymous leaderboard name</div>
          <div style={{fontSize:17,fontWeight:800,color:"#EEEEEE",fontFamily:"monospace",letterSpacing:1}}>{publicName}</div>
          <div style={{fontSize:11,color:"#6B7280",marginTop:4,lineHeight:1.4}}>This is how you appear on global rankings. Your real name is only shown to your groups.</div>
        </div>

        <button onClick={handleSignup} disabled={loading} style={{width:"100%",padding:"15px 0",borderRadius:12,background:"#00ADB5",border:"none",color:"#fff",fontSize:16,fontWeight:700,cursor:loading?"default":"pointer",opacity:loading?0.6:1,marginBottom:12}}>
          {loading?"Creating account...":"Create Account"}
        </button>
        <button onClick={()=>{setMode("login");setError("");setPassword("");}} style={{width:"100%",padding:"12px 0",borderRadius:12,background:"transparent",border:"none",color:"#6B7280",fontSize:14,cursor:"pointer"}}>
          Already have an account? <span style={{color:"#00ADB5",fontWeight:600}}>Log in</span>
        </button>
        <button onClick={()=>{setMode("welcome");setError("");}} style={{width:"100%",padding:"8px 0",background:"transparent",border:"none",color:"#4A6070",fontSize:13,cursor:"pointer",marginTop:4}}>← Back</button>
      </div>
    </div>
...
window.TriviaApp = function App(){
  const [user,setUser]=useState(()=>{
    try{const s=localStorage.getItem("uncl_user");return s?JSON.parse(s):null;}catch(e){return null;}
  });
  ...
  return(
    ...
    <>
      ...
      `}</style>
      {!user&&(
        <OnboardingScreen c={c} onComplete={u=>{try{localStorage.setItem("uncl_user",JSON.stringify(u));}catch(e){}setUser(u);setScreen("home");}}/>
      )}
      {user&&<div style={{maxWidth:430,margin:"0 auto",paddingBottom:showNav?64:0,minHeight:"100vh",background:c.bg}}>
        ...
        {screen==="profile"     &&<ErrorBoundary><ProfileScreen profile={profile} groups={groups} onBack={()=>nav("home")} c={c} user={user} onUpdateUser={setUser} onLogout={()=>{try{localStorage.removeItem("uncl_user");}catch(e){}setUser(null);setScreen("home");setModeScores({});}}/></ErrorBoundary>}
